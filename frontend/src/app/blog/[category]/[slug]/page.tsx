...
import { BlogLeadCapture } from '../../[slug]/BlogPostClient';
import publishedBlogs from '@/data/published-blogs.json';
import { classify, getRelated, BLOG_CATEGORIES, NEWS_CATEGORIES, type PublishedPost, type ContentType } from '@/lib/content-taxonomy';

// Shared with /news/[category]/[slug]
...
export function generateStaticParams() {
  ...
  if (params.length === 0) return [{ category: 'market-updates', slug: '_' }];
  return params;
}
...
export async function generateMetadata({
  ...
  const post = ALL_CLASSIFIED.find(
    (p) => p.contentType === 'blog' && p.category.slug === category && p.slug === slug,
  );
  if (!post) return { title: 'Not Found' };
  return {
    title: `${post.title} | MesaHomes`,
        ...
        <article className="bg-paper px-4 py-16">
          <div className="mx-auto" style={{ maxWidth: '65ch' }}>
            <nav className="mb-4 text-sm text-text-light">
              ...
              {' / '}
              <Link href={`/blog/${post.category.slug}/`} className="hover:text-primary">
                {post.category.title}
              </Link>
            </nav>
            ...
            {related.length > 0 && (
              <aside className="mt-12 border-t border-warm-border pt-8">
                <h3 className="mb-4 font-heading text-lg font-bold text-charcoal">Related Articles</h3>
